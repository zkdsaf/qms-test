                  ...
                  <template #header>
                    <n-space>
                      ...
                      <h2 class="text-red-500 text-lg font-bold">
                        重复申请 AlarmRule(WE1,WE2,WE3,WE4,WE5)
                      </h2>
                    </n-space>
                  </template>
...
const computedTitleName = () => {
  const { uploadType, ecoaType } = route.query
  const title =
    uploadType === 'batch' ? '按批次上传COA信息' : '按检测样本上传COA信息'
  const ecoaTitle = ecoaType === 'manual' ? '人工' : '系统'
  return `${ecoaTitle}    ${title} `
}
// 表单数据
...
const basicColumns = [
  { title: '名称', key: 'name1', align: 'center', width: 150 },
  { title: '数据填写及内容', key: 'content1', align: 'center', width: 150 },
  {
    title: '与Spec Value差异',
    key: 'specValueDiff1',
    align: 'center',
    width: 150,
    render: (row) => <span class="text-red-500">{row.specValueDiff1}</span>,
  },
  { title: '名称', key: 'name2', align: 'center', width: 150 },
  { title: '数据填写及内容', key: 'content2', align: 'center', width: 150 },
  {
    title: '与Spec Value差异',
    key: 'specValueDiff2',
    align: 'center',
    width: 150,
    render: (row) => <span class="text-red-500">{row.specValueDiff2}</span>,
  },
]
...
const generateBasicData = () => {
  ...
  for (let i = 0; i < length; i++) {
    data.push({
      ...
      content1: `数据填写及内容${i}`,
      nameDiff1: `名称差异${i}`,
      specValueDiff1: `与Spec Value差异${i}`,
      name2: `名称${i}`,
      content2: `数据填写及内容${i}`,
      nameDiff2: `名称差异${i}`,
      specValueDiff2: `与Spec Value差异${i}`,
    })
  }
  return data
}
