<template>
  <div
    ...
    <div class="login-wrapper flex w-[900px] h-[500px] bg-white bg-opacity-95 rounded-2xl shadow-2xl animate-slide-in">
      <!-- 左侧装饰区域 -->
      <div
        ...
        <div class="particle-container absolute top-0 left-0 w-full h-full overflow-hidden">
          <div class="particle absolute bg-white bg-opacity-30 rounded-full animate-float" v-for="n in 10" :key="n"
      ...
      <n-card class="login-card w-[400px] p-8 flex flex-col justify-center border-none shadow-none">
        ...
        <n-form ref="formRef" :model="formState" class="login-form" @keydown.enter="onFinish">
          ...
          <n-form-item label="密码" path="password" :rule="{
            required: true,
            message: '请输入密码!',
            trigger: ['input', 'blur'],
          }">
            <n-input v-model:value="formState.password" type="password" placeholder="请输入密码" show-password-on="click"
              size="large">
              <template #prefix>
                <n-icon size="18" class="text-gray-500">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M16 8v-2a4 4 0 0 0-8 0v2m-4 2h16v8a2 2 0 0 1-2 2h-12a2 2 0 0 1-2-2v-8z" />
                  </svg>
                </n-icon>
              </template>
            </n-input>
          ...
          <n-form-item>
            <n-button type="primary" size="large" :loading="loading" class="w-full" @click="onFinish">
              登录
            </n-button>
          </n-form-item>
        </n-form>
